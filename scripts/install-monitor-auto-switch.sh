...
# - Installs a udev monitor-hotplug rule to handle DRM change events
...
Expected files in --src:
  hypr-utils.sh, lid-open.sh, lid-close.sh, check-lid-on-startup.sh, monitor-hotplug.sh
...
install_from_repo() {
  ...
  if [ -f "$REPO_ACPI_DIR/monitor-hotplug.sh" ]; then
